...
%   poly_rep_rec.m
%
%   homa dot-pattern REP condition
%   with transfer recognition phase
%   transfer phase presents the 15 old-medium distortions,
%     15 new medium distortions, the three prototypes, 
%       and 6 foils (2 from each category)
%
clear all;
data_location=[pwd '\data\'];
subid=input(' subject # ');
filename=[data_location 'polyrep' num2str(subid) '.txt'];
allvars=[data_location 'polyrep' num2str(subid)];
if ~exist(filename)
    ...
    for block=1:nblocktest
        ...
        for trial=1:ntrans
            ...
            corr=0;
            if resp == 1 && itemtype == 1
                corr=1;
            end
            if resp == 2 && itemtype ~= 1
                corr=1;
            end                     
            Screen('Flip',wind1);
            if corr == 1
